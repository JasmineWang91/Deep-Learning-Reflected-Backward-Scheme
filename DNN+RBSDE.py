...
for n in range(N):
    X[n+1] = X[n] + (5 *I*H)/d + 3*sqrth*(np.dot(noise[n],I_mat))/(sqrtd)
    L[n+1] = norm(X[n+1])
    V[n+1] = norm(X[n+1])+1/2
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            if n == N-1:
                ...
                val_loss = lossYZ.eval(feed_dict={Xsc: NoiseValidation[ep1: ep2], Ynext_op: Y_op[ep1: ep2], Xn: X[n]*onesarray1, Bn: noise[n]*onesarray1})
            ...
            for batch in range(n_batches):
                ...
                if n==N-1:
                    ...
                    sess.run(training_op_Y, feed_dict={learning_rate: init_learning_rate, Xsc: NoiseTraining[ind1:ind2], Ynext_op: Y_op[ind1:ind2], Xn: X[n] * onesarray2, Bn: noise[n] * onesarray2})
                    sess.run(training_op_Z, feed_dict={learning_rate: init_learning_rate, Xsc: NoiseTraining[ind1:ind2], Ynext_op: Y_op[ind1:ind2], Xn: X[n] * onesarray2, Bn: noise[n] * onesarray2})
